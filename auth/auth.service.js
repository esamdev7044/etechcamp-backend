...
exports.registerUser = async ({ email, password }) => {
  ...
  try {
    ...
    const existing = await repo.findUserByEmail(pool, email);
    if (existing) {
...
exports.loginUser = async (req, { email, password }) => {
  ...
  try {
    const user = await repo.findUserByEmail(pool, email);
    ...
    if (!match) {
      await repo.incrementFailedAttempts(pool, user.id);

      if (user.failed_attempts + 1 >= MAX_FAILED_ATTEMPTS) {
        const lockUntil = new Date(Date.now() + LOCK_TIME_MINUTES * 60 * 1000);
        await repo.lockUser(pool, user.id, lockUntil);
      }

      throw new AppError("Invalid credentials", 401);
    }

    await repo.resetFailedAttempts(pool, user.id);
